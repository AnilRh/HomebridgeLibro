...
class CachedPetLibroAPI {
  ...
  /**
   * Force stop feed with cache invalidation (fallback method)
   */
  async forceStopFeed(deviceId) {
    const result = await this.api.forceStopFeed(deviceId);
    
    if (result.success) {
      // Invalidate device status cache after force stopping feed
      this.invalidateCache(`realInfo:${deviceId}`);
      this.invalidateCache(`feedingStatus:${deviceId}`);
    }
