...
class PolarFeeder extends BaseFeeder {
  ...
  setupAudioControl() {
    ...
    this.audioService = this.accessory.getService('Play Audio') ||
      this.accessory.addService(this.platform.api.hap.Service.Switch, 'Play Audio', 'audio');

    this.audioService.setCharacteristic(this.platform.api.hap.Characteristic.Name, 
      `${this.config.name || 'Polar Feeder'} Audio`);

    this.audioService.getCharacteristic(this.platform.api.hap.Characteristic.On)
      .onSet(this.handleAudioSwitch.bind(this));
  ...
  async handleAudioSwitch(value) {
    if (!value) {
      this.log('Audio switch turned OFF (ignored - this is a momentary switch)');
      return;
    }
